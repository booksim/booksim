...
const bool MATLAB_OUTPUT        = false;  // output data in MATLAB friendly format
const bool DISPLAY_LAT_DIST     = false; // distribution of packet latencies
const bool DISPLAY_HOP_DIST     = false;  // distribution of hop counts
const bool DISPLAY_PAIR_LATENCY = false;  // avg. latency for each s-d pair
				      ...
				      int cl, int time )
{
  ...
  dest = _traffic_function( source, _net->NumDests( ) );
  ...
  for ( int i = 0; i < size; ++i ) {
    ...
    f->true_tail = false;
    if ( ( i == ( size - 1 ) ) || ( split_tail ) ) { // Tail flit
      f->tail = true;
...
void TrafficManager::Run( )
{
  ...
  if ( _reorder ) {
    cout << "Overall average reorder latency = " << _rob_latency->Average( ) << endl;
    cout << "Overall average reorder size - " << _rob_size->Average( ) << endl;
